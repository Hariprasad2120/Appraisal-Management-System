                  ...
                  <div key={tl.id} className="rounded-xl border border-border bg-card">
                    <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
                      <div>
                        ...
                        <p className="text-[11px] text-muted-foreground">{tl.department ?? "No department"} · TL</p>
                      </div>
                      ...
                      {owned.map((emp) => (
                        <div key={emp.id} className="flex items-center justify-between gap-3 px-4 py-2.5">
                          <div className="min-w-0">
                            ...
                            <p className="truncate text-[11px] text-muted-foreground">{emp.department ?? "—"} · {emp.designation ?? emp.role}</p>
                          </div>
                  ...
                  <div key={mgr.id} className="rounded-xl border border-border bg-card">
                    <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
                      <div>
                        ...
                        <p className="text-[11px] text-muted-foreground">{mgr.department ?? "No department"} · Manager</p>
                      </div>
        ...
        <FadeIn delay={0.08}>
          <div className="space-y-4">
                          ...
                          return (
                            <tr key={div.id} className="hover:bg-muted/20">
                              <td className="px-4 py-3 pl-8 text-foreground">
                                <span className="text-muted-foreground mr-1.5">└</span>{div.name}
                  ...
                  {rootDepts.length === 0 && (
                    <tr>
                      <td colSpan={4} className="px-4 py-10 text-center text-muted-foreground">
                        No KPI departments found. Configure them in{" "}
                        <Link href="/admin/kpi?tab=departments" className="text-primary hover:underline">Department KPI</Link>.
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            <p className="text-[11px] text-muted-foreground">
              Manager and TL columns show users whose KPI department matches this row. Assign employees to departments in{" "}
              <Link href="/admin/kpi?tab=departments" className="text-primary hover:underline">Department KPI</Link>{" "}
              and set TL/Manager assignments in the TL and Manager tabs.
            </p>
          </div>
        </FadeIn>
      )}
