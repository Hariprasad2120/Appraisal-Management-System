...
export default async function ReviewerDashboard() {
  ...
  const [assignments, reviewer, recentNotifs, myOwnCycles, activeTickets] = await Promise.all([
    ...
    prisma.appraisalCycle.findMany({
      ...
      take: 1,
      include: {
        self: { select: { status: true, editableUntil: true, submittedAt: true, locked: true } },
        assignments: { select: { availability: true, role: true } },
        ratings: { select: { reviewerId: true, averageScore: true } },
        decision: { select: { finalAmount: true, finalRating: true, decidedAt: true } },
      },
    }),
    prisma.ticket.findMany({
      where: { raisedById: session.user.id, status: { in: ["OPEN", "IN_PROGRESS"] } },
    ...
    >
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          ...
          {recentNotifs.length > 0 && (
            ...
            >
              ...
              {recentNotifs.length} unread notification{recentNotifs.length !== 1 ? "s" : ""}
            </Link>
          )}
        </div>
      </FadeIn>
      ...
      <div className="grid grid-cols-1 gap-5 items-start">
          ...
          {activeTickets.length > 0 && (
            <FadeIn delay={0.1}>
              <SectionCard
                ...
                action={<Link href="/tickets" className="text-[11px] text-[#0e8a95] hover:underline">View all</Link>}
              >
              ...
              >
                <div className="px-5 pb-4 pt-1 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground">Cycle type</span>
                    <span className="text-xs font-semibold text-[#0e8a95]">{myOwnCycle.type}</span>
                  </div>
                  <div className="flex items-center justify-between">
        ...
        <div className="grid min-w-0 gap-5 md:grid-cols-2 xl:grid-cols-3">
            ...
            <FadeIn delay={0.1}>
              ...
              >
                <div className="px-5 pb-4 pt-1 grid grid-cols-2 gap-3">
                  <SalaryField label="CTC (Annual)" value={Number(reviewer.salary.ctcAnnum)} />
                  <SalaryField label="Gross (Annual)" value={Number(reviewer.salary.grossAnnum)} />
                  <SalaryField label="Basic" value={Number(reviewer.salary.basic)} />
                  <SalaryField label="HRA" value={Number(reviewer.salary.hra)} />
                  {Number(reviewer.salary.conveyance) > 0 && (
                    <SalaryField label="Conveyance" value={Number(reviewer.salary.conveyance)} />
                  )}
                  {Number(reviewer.salary.fixedAllowance) > 0 && (
                    <SalaryField label="Fixed Allowance" value={Number(reviewer.salary.fixedAllowance)} />
                  )}
                </div>
              </SectionCard>
            </FadeIn>
          ...
          {/* Quick links */}
          <FadeIn delay={0.18}>
            <SectionCard title="Quick Links" icon={<ArrowRight className="size-3.5" />}>
              <div className="px-5 pb-4 pt-1 flex flex-col gap-2">
                {[
                  { href: "/reviewer", label: "All Assignments" },
...
function SalaryField({ label, value }: { label: string; value: number }) {
  return (
    <div className="bg-muted/40 rounded-lg p-2.5 border border-border">
      ...
      <div className="text-sm font-bold text-foreground">{formatCurrency(value)}</div>
    </div>
  );
}
...
function ReviewRow({
  ...
  const statusConfig = {
    action:      { label: "Set Availability", dot: "bg-amber-400",   badge: "bg-amber-50 text-amber-700 dark:bg-amber-950/30 dark:text-amber-400 border-amber-200 dark:border-amber-800" },
    rate:        { label: "Rating Open",      dot: "bg-[#0e8a95]",   badge: "bg-[#0e8a95]/10 text-[#0e8a95] border-[#0e8a95]/20" },
    completed:   { label: "Completed",        dot: "bg-green-500",   badge: "bg-green-50 text-green-700 dark:bg-green-950/30 dark:text-green-400 border-green-200 dark:border-green-800" },
    waiting:     { label: "Waiting",          dot: "bg-slate-300",   badge: "bg-muted text-muted-foreground border-border" },
    unavailable: { label: "Not Available",    dot: "bg-slate-400",   badge: "bg-muted text-muted-foreground border-border" },
  }[rowStatus];

  let actionHref = `/reviewer/${assignment.cycle.id}`;
  let actionLabel = "View Details";
  let actionStyle = "bg-muted text-muted-foreground hover:bg-muted/80 border border-border";
    ...
    <tr className="hover:bg-muted/30 transition-colors">
      ...
      <td className="px-3 py-3.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] text-muted-foreground">{doneReviewers}/{totalReviewers}</span>
        </div>
      </td>
      ...
      <td className="px-3 py-3.5">
        {visibleAverage !== null ? (
          ...
          <span className="text-[11px] text-muted-foreground italic">Pending peers</span>
        ) : (
        ...
        {assignment.cycle.self ? (
          <div>
            ...
            {now > assignment.cycle.self.editableUntil && (
              <div className="text-[10px] text-red-500 font-medium mt-0.5">Passed</div>
            )}
          </div>
        ) : (
          <span className="text-muted-foreground text-xs">—</span>
