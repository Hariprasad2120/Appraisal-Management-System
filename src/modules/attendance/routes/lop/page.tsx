                ...
                >
                  ...
                  {employees.map((emp) => (
                    <option key={emp.id} value={emp.id}>
                      {emp.employeeNumber ? `#${emp.employeeNumber} â€” ` : ""}{emp.name}
                    </option>
                  ))}
                </select>
              </div>
      ...
      <FadeIn delay={0.15}>
        <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between">
            <span className="text-sm font-semibold text-foreground">LOP Records â€” {month}</span>
                ...
                <tbody className="divide-y divide-border">
                  {records.map((rec) => (
                    <tr key={rec.id} className="hover:bg-muted/40 transition-colors">
                      ...
                      <td className="px-4 text-xs text-muted-foreground">{rec.employee.department ?? "â€”"}</td>
                      <td className="px-4">
                        <span className="text-sm font-semibold text-rose-600 dark:text-rose-400">
                          {Number(rec.lopDays).toFixed(1)}
                        </span>
                      </td>
                      <td className="px-4 text-xs text-muted-foreground">{rec.remarks ?? "â€”"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
