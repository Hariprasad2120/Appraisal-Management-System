...
import { saveHrMomAction } from "./reviewer-mom-actions";
...
export default async function HrMomDetailPage({ params }: { params: Promise<{ cycleId: string }> }) {
  ...
  // HR can only record/view MOM for cycles they are assigned to
  ...
  const hrMom = cycle.moms[0] ?? null;

  const autoContent = `Minutes of Meeting — Appraisal Review (HR Notes)
...
HR Observations:
  [Record HR notes and compliance observations here]
        ...
        <FadeIn delay={0.1}>
          <Card className="border-0 shadow-sm">
            <CardContent className="p-5">
              <MomContentEditor
                cycleId={cycleId}
                existingContent={hrMom?.content ?? autoContent}
                isNew={!hrMom}
                saveAction={saveHrMomAction}
                successMessage={!hrMom ? "MOM recorded" : "MOM updated"}
              />
            </CardContent>
          </Card>
        </FadeIn>
