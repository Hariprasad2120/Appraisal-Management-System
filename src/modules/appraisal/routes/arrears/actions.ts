...
export async function approveArrearAction(input: z.infer<typeof approveSchema>): Promise<Result> {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.notification.create({
      data: {
        ...
        message: `Your arrear of â‚¹${Number(arrear.arrearAmount).toLocaleString("en-IN")} for ${arrear.arrearDays} days has been approved. It will be credited in ${payoutStr}.`,
        link: "/appraisal",
        persistent: true,
        critical: true,
      },
      ...
      adminUsers.map((u) =>
        tx.notification.create({
          data: {
            userId: u.id,
            type: "ARREAR_APPROVED",
            message: `Arrear for ${arrear.cycle.user.name} approved. â‚¹${Number(arrear.arrearAmount).toLocaleString("en-IN")} for ${arrear.arrearDays} days â€” payout: ${payoutStr}.`,
            link: `/admin/arrears`,
            persistent: false,
          },
        })
  ...
  revalidatePath("/appraisal/arrears");
  revalidatePath("/appraisal/arrears");
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.notification.create({
      data: {
        userId: arrear.userId,
        type: "ARREAR_PAID",
        message: `Your arrear of â‚¹${Number(arrear.arrearAmount).toLocaleString("en-IN")} has been processed and credited.`,
        link: "/appraisal",
        persistent: true,
        critical: true,
      },
