...
import { getHrmsEmployeePath, getHrmsEmployeesPath } from "@/modules/hrms/lib/routes";
...
export async function createEmployeeAction(formData: FormData) {
  ...
  redirect(getHrmsEmployeePath(created.id));
}
...
export async function updateEmployeeAction(id: string, formData: FormData) {
  ...
  revalidatePath("/hrms/employees");
  revalidatePath(getHrmsEmployeePath(id));
  redirect(getHrmsEmployeePath(id));
}
...
export async function upsertSalaryAction(userId: string, formData: FormData) {
  ...
  revalidatePath(getHrmsEmployeePath(userId));
  redirect(getHrmsEmployeePath(userId));
}
